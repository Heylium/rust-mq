...
use log::{error, info};
use prost::Message;
use raft::eraftpb::{ConfChange, ConfChangeType, Entry, EntryType, Message as raftPreludeMessage, MessageType, Snapshot};
...
use crate::raft::apply::{RaftMessage, RaftResponseMessage};
...
use crate::raft::storage::RaftRocksDBStorage;
...
impl RaftMachine {
    ...
    async fn on_ready(&mut self, raft_node: &mut RawNode<RaftRocksDBStorage>) {
        ...
        // If there is a change in HardState, such as a re-vote,
        ...
        if let Some(commit) = light_rd.commit_index() {
            ...
            raft_node.mut_store().set_hard_state_commit(commit).unwrap();
        }
        ...
        for entry in entries {
            ...
            match deserialize(entry.get_context()) {
                Ok(seq) => match self.resp_channel.remove(&seq) {
                    Some(chan) => match chan.send(RaftResponseMessage::Success) {
                        Ok(_) => {}
                        Err(_) => {
                            error!("commit entry Fails to return data to chan. chan may have been closed");
                        }
    ...
    pub async fn send_peer_message(&self, id: u64, msg: Vec<u8>) {
        if let Some(node) = self.placement_cluster.read().unwrap().get_node_by_id(id) {
            ...
            tokio::spawn(async move {
                match send.send(peer_msg).await {
                    Ok(_) => {},
                    Err(e) => error!(
