...
use raft::{Error, GetEntriesContext, RaftState, Result as RaftResult, StorageError};
...
impl RaftRocksDBStorage {
    ...
    pub fn set_hard_state_commit(&mut self, hs: u64) -> RaftResult<()> {
        let store = self.core.write().unwrap();
...
impl RaftStorage for RaftRocksDBStorage {
    ...
    fn entries(&self, low: u64, high: u64, max_size: impl Into<Option<u64>>, context: GetEntriesContext) -> RaftResult<Vec<Entry>> {
        let core = self.read_lock();
    ...
    fn term(&self, idx: u64) -> RaftResult<u64> {
        ...
        Ok(core.snapshot_metadata.term)
    }
